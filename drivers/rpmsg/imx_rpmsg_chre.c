...
static const char chre_ack_message[10] = "CHRE";
...
enum chre_state_t chre_state = SCP_CHRE_UNINIT;
			...
			void *priv, u32 addr)
{
	...
	if (header->cmd == CHRE_MESSAGE_DATA) {
		if (eptdev->message_buf == NULL) {
			eptdev->message_buf = kzalloc(total_size, GFP_ATOMIC);
			if (!eptdev->message_buf) {
				dev_err(dev, "failed to allocate message buffer\n");
				return -ENOMEM;
			}
		}
		memcpy(eptdev->message_buf + eptdev->message_offset, buf + sizeof(struct chre_message_head), len);
		eptdev->message_offset += len;
		...
		if (eptdev->message_buf != NULL)
			skb_put_data(skb, eptdev->message_buf, eptdev->message_offset);
		skb_put_data(skb, buf + sizeof(struct chre_message_head), len);
		spin_lock(&eptdev->queue_lock);
		skb_queue_tail(&eptdev->queue, skb);
		spin_unlock(&eptdev->queue_lock);
		/* finish release memory */
		kfree(eptdev->message_buf);
		eptdev->message_buf = NULL;
		eptdev->message_offset = 0;
...
static int chre_send_power_state(struct device *dev, unsigned char state)
{
	...
	if (!pdev) {
		dev_err(dev, "failed to get platform device\n");
		return -ENODEV;
	}
	struct platform_rpmsg_data* pdata = platform_get_drvdata(pdev);
	if (!pdata || !pdata->eptdev || !pdata->eptdev->ept) {
		dev_err(dev, "platform data or eptdev not available\n");
		return -ENODEV;
	}

	msg.power_state = state;
	msg.header.cate = CHRE_CATEGORY;
	msg.header.major = IMX_RMPSG_MAJOR;
	msg.header.minor = IMX_RMPSG_MINOR;
	msg.header.type = CHRE_RPMSG_REQUEST;
	...
	mutex_lock(&pdata->eptdev->ept_lock);
...
static int rpmsg_chre_probe(struct rpmsg_device *rpdev)
{
	...
	if (sp != NULL) {
		...
		if (pdev != NULL) {
			...
			dev_err(dev, "chre device not found\n");
			return -1;
		}
	} else {
		dev_err(dev, "chre node not found\n");
		return -1;
	}
