...
static int imx8m_blk_ctrl_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < bc_data->num_domains; i++) {
		...
		pm_genpd_add_subdomain(pd_to_genpd(domain->power_dev->pm_domain), &domain->genpd);
		pm_genpd_add_subdomain(pd_to_genpd(bc->bus_power_dev->pm_domain), pd_to_genpd(domain->power_dev->pm_domain));
	}
