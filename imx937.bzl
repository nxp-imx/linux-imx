...
# Copyright 2026 NXP
# Bazel build configuration for i.MX 937 (frdm_937)
...
# =============================================================================
# i.MX 937 Module Lists
# Extracted from device/nxp/imx9/frdm_937/SharedBoardConfig.mk
# =============================================================================

# IMX_ANDROID_FIRST_STAGE_MODULES - modules loaded at first stage boot
_IMX937_FIRST_STAGE_MODULES = [
    # HWMON & SCMI
...
_IMX937_RECOVERY_ADDITION_MODULES = [
    # Backlight
...
_IMX937_VENDOR_RAMDISK_MODULES = _IMX937_FIRST_STAGE_MODULES + _IMX937_RECOVERY_ADDITION_MODULES

# Mali GPU modules (USE_GPU_DRIVERS=mali)
_IMX937_MALI_GPU_MODULES = [
...
# Note: vendor_dlkm also includes _IMX937_RECOVERY_ADDITION_MODULES
_IMX937_VENDOR_DLKM_MODULES = [
    # Media / Camera
...
_IMX937_EXT_VENDOR_DLKM_MODULES = [
    "mlan.ko",
    "moal.ko",
]

# Implicit modules (optional - build continues if missing)
# imx_rpmsg_chre.ko is guarded by ifeq($(ENABLE_CONTEXTHUB), true) in
# SharedBoardConfig.mk, so it is treated as optional here.
_IMX937_IMPLICIT_MODULES = [
...
_IMX937_EXT_MODULES = [
    "//nxp-mwifiex:mwifiex_modules_imx937",
]

# Mesa GPU modules (USE_GPU_DRIVERS=mesa)
_IMX937_MESA_GPU_MODULES = [
    "drivers/gpu/drm/drm_exec.ko",
    "drivers/gpu/drm/drm_gpuvm.ko",
    "drivers/gpu/drm/scheduler/gpu-sched.ko",
    "drivers/gpu/drm/panthor/panthor.ko",
...
_IMX937_IN_TREE_MODULES = _IMX937_VENDOR_RAMDISK_MODULES + _IMX937_VENDOR_DLKM_MODULES + _IMX937_MALI_GPU_MODULES

# Module load lists for initramfs
# BOARD_VENDOR_RAMDISK_KERNEL_MODULES_LOAD (normal boot, first stage only)
_IMX937_MODULES_LIST = _IMX937_FIRST_STAGE_MODULES

# BOARD_VENDOR_RAMDISK_RECOVERY_KERNEL_MODULES_LOAD (recovery mode, all vendor ramdisk)
_IMX937_MODULES_RECOVERY_LIST = _IMX937_VENDOR_RAMDISK_MODULES

# Charger mode module list (same as recovery)
_IMX937_MODULES_CHARGER_LIST = _IMX937_VENDOR_RAMDISK_MODULES

# =============================================================================
# i.MX 937 Device Tree Blobs (DTB)
# Extracted from device/nxp/imx9/frdm_937/BoardConfig.mk TARGET_BOARD_DTS_CONFIG
# =============================================================================

# All DTB files needed for DTBO image generation
# Format: arch/arm64/boot/dts/freescale/<filename>.dtb
# Composite DTBs are assembled by the kernel Makefile from base + .dtbo overlays;
# kleaf only needs the final .dtb names listed here.
_IMX937_DTB_OUTS = [
    "arch/arm64/boot/dts/freescale/imx937-frdm-dual-ap1302.dtb",
    "arch/arm64/boot/dts/freescale/imx937-frdm-aud-hat.dtb",
    "arch/arm64/boot/dts/freescale/imx937-frdm-boe-wxga-lvds-panel.dtb",
    "arch/arm64/boot/dts/freescale/imx937-frdm-waveshare-7inch-c-panel.dtb",
]


def define_imx937():
    """Define Bazel targets for i.MX 937 kernel build."""
    ...
    write_file(
        name = "imx937_modules_list",
        out = "imx937_modules.txt",
        content = [m.split("/")[-1] for m in _IMX937_MODULES_LIST] + [""],
    )

    # Modules list for recovery mode
    write_file(
        name = "imx937_modules_recovery_list",
        out = "imx937_modules_recovery.txt",
        content = [m.split("/")[-1] for m in _IMX937_MODULES_RECOVERY_LIST] + [""],
    )

    # Modules list for charger mode
    write_file(
        name = "imx937_modules_charger_list",
        out = "imx937_modules_charger.txt",
        content = [m.split("/")[-1] for m in _IMX937_MODULES_CHARGER_LIST] + [""],
    )

    # Modules list for vendor ramdisk (initramfs)
    # Combines first stage + recovery modules
    write_file(
        name = "imx937_vendor_ramdisk_modules_list",
        out = "imx937_vendor_ramdisk_modules.txt",
        content = [m.split("/")[-1] for m in _IMX937_VENDOR_RAMDISK_MODULES] + [""],
    )

    # Explicit module load order for vendor ramdisk
    # Matches the order defined in _IMX937_MODULES_LIST
    write_file(
        name = "imx937_modules_load_order",
        out = "imx937_modules.load",
        content = [m.split("/")[-1] for m in _IMX937_MODULES_LIST] + [""],
    )
    ...
    write_file(
        name = "imx937_vendor_dlkm_modules_list",
        out = "imx937_vendor_dlkm_modules.txt",
        content = [m.split("/")[-1] for m in _IMX937_RECOVERY_ADDITION_MODULES + _IMX937_VENDOR_DLKM_MODULES + _IMX937_MALI_GPU_MODULES] + _IMX937_EXT_VENDOR_DLKM_MODULES + [""],
    )

    # Explicit module load order for vendor_dlkm
    # Matches the order defined in recovery + vendor dlkm + GPU modules
    write_file(
        name = "imx937_vendor_dlkm_modules_load_order",
        out = "imx937_vendor_dlkm_modules.load",
        content = [m.split("/")[-1] for m in _IMX937_RECOVERY_ADDITION_MODULES + _IMX937_VENDOR_DLKM_MODULES + _IMX937_MALI_GPU_MODULES] + _IMX937_EXT_VENDOR_DLKM_MODULES + [""],
    )

    # Modules list for vendor_dlkm (mesa GPU mode)
    write_file(
        name = "imx937_mesa_vendor_dlkm_modules_list",
        out = "imx937_mesa_vendor_dlkm_modules.txt",
        content = [m.split("/")[-1] for m in _IMX937_RECOVERY_ADDITION_MODULES + _IMX937_VENDOR_DLKM_MODULES + _IMX937_MESA_GPU_MODULES] + _IMX937_EXT_VENDOR_DLKM_MODULES + [""],
    )

    # Explicit module load order for vendor_dlkm (mesa GPU mode)
    write_file(
        name = "imx937_mesa_vendor_dlkm_modules_load_order",
        out = "imx937_mesa_vendor_dlkm_modules.load",
        content = [m.split("/")[-1] for m in _IMX937_RECOVERY_ADDITION_MODULES + _IMX937_VENDOR_DLKM_MODULES + _IMX937_MESA_GPU_MODULES] + _IMX937_EXT_VENDOR_DLKM_MODULES + [""],
    )
    ...
    kernel_build(
        ...
        # Use gki_defconfig + imx937 fragment
        defconfig = "arch/arm64/configs/gki_defconfig",
        pre_defconfig_fragments = [
            "arch/arm64/configs/imx937_gki.fragment",
        ],
        make_goals = [
            "Image",
            "Image.lz4",
        ...
        module_outs = _IMX937_IN_TREE_MODULES,
        # Implicit modules (optional - build continues if missing)
        module_implicit_outs = _IMX937_IMPLICIT_MODULES,
        # Symbol list for ABI
    ...
    kernel_abi(
        name = "imx937_abi",
        kernel_build = ":imx937",
        kernel_modules = _IMX937_EXT_MODULES,
        module_grouping = False,
        kmi_symbol_list_add_only = True,
    )

    kernel_modules_install(
        name = "imx937_modules_install",
        kernel_build = ":imx937",
        kernel_modules = _IMX937_EXT_MODULES,
    )
    ...
    # Contains _IMX937_VENDOR_RAMDISK_MODULES with separate load lists per boot mode
    initramfs(
        name = "imx937_initramfs",
        kernel_modules_install = ":imx937_modules_install",
        ramdisk_compression = "lz4",
        # Normal boot: first stage modules only
        modules_list = ":imx937_modules_list",
        # Explicit load order matching _IMX937_MODULES_LIST
        modules_load = ":imx937_modules_load_order",
        # Recovery mode: all vendor ramdisk modules
        modules_recovery_list = ":imx937_modules_recovery_list",
        # Charger mode: same as recovery
        modules_charger_list = ":imx937_modules_charger_list",
    ...
    vendor_boot_image(
        name = "imx937_vendor_boot",
        outs = [
            "ramdisk.lz4",
        ],
        initramfs = ":imx937_initramfs",
        kernel_build = ":imx937",
        unpack_ramdisk = True,
        ramdisk_compression = "lz4",
        vendor_boot_name = "vendor_boot",
    ...
    pkg_files(
        name = "imx937_kernel_files",
        srcs = [
            ":imx937",
            ":imx937_modules_install",
        ],
    ...
    pkg_files(
        name = "imx937_mesa_vendor_dlkm_files",
        srcs = [
            ":imx937_mesa_vendor_dlkm",
        ],
        strip_prefix = strip_prefix.files_only(),
        visibility = ["//visibility:private"],
    )
    ...
    _dtb_srcs = [
        ":imx937/" + f for f in _IMX937_DTB_OUTS
    ]

    pkg_files(
        name = "imx937_dtb_files",
        srcs = _dtb_srcs,
        strip_prefix = strip_prefix.from_pkg("imx937/arch/arm64/boot/dts/freescale"),
        visibility = ["//visibility:private"],
    )
    ...
    # Command: tools/bazel run //kernel_imx:imx937_dist
    pkg_install(
        name = "imx937_dist",
        srcs = [
            ":imx937_kernel_files",
            ":imx937_vendor_boot_files",
            ":imx937_vendor_dlkm_files",
            ":imx937_boot_files",
            ":imx937_system_dlkm_files",
            ":imx937_dtb_files",
        ],
        destdir = "out/imx_frdm_937_aarch64/dist",
    )

    # Vendor boot only (initramfs.img)
    # Command: tools/bazel run //kernel_imx:imx937_vendor_boot_dist
    pkg_install(
        name = "imx937_vendor_boot_dist",
        srcs = [
            ":imx937_kernel_files",
            ":imx937_vendor_boot_files",
        ],
    ...
    # Command: tools/bazel run //kernel_imx:imx937_vendor_dlkm_dist
    pkg_install(
        name = "imx937_vendor_dlkm_dist",
        srcs = [
            ":imx937_kernel_files",
            ":imx937_vendor_dlkm_files",
        ],
    ...
    # GKI boot.img only
    # Command: tools/bazel run //kernel_imx:imx937_boot_dist
    pkg_install(
        name = "imx937_boot_dist",
        srcs = [
            ":imx937_boot_files",
        ],
        destdir = "out/imx_frdm_937_aarch64/dist",
    ...
    # Command: tools/bazel run //kernel_imx:imx937_system_dlkm_dist
    pkg_install(
        name = "imx937_system_dlkm_dist",
        srcs = [
            ":imx937_system_dlkm_files",
        ],
        destdir = "out/imx_frdm_937_aarch64/dist",
    ...
    # Command: tools/bazel run //kernel_imx:imx937_gki_dist
    pkg_install(
        name = "imx937_gki_dist",
        srcs = [
            ":imx937_boot_files",
            ":imx937_system_dlkm_files",
        ],
    ...
    # Command: tools/bazel run //kernel_imx:imx937_dtb_dist
    pkg_install(
        name = "imx937_dtb_dist",
        srcs = [
            ":imx937_dtb_files",
        ],
        destdir = "out/imx_frdm_937_aarch64/dist",
    ...
    # Command: tools/bazel run //kernel_imx:imx937_mesa_dist
    pkg_install(
        name = "imx937_mesa_dist",
        srcs = [
            ":imx937_kernel_files",
            ":imx937_vendor_boot_files",
            ":imx937_mesa_vendor_dlkm_files",
            ":imx937_boot_files",
            ":imx937_system_dlkm_files",
            ":imx937_dtb_files",
        ],
        destdir = "out/imx_frdm_937_aarch64_mesa/dist",
    )

    # Vendor DLKM only (mesa — panthor GPU modules)
    # Command: tools/bazel run //kernel_imx:imx937_mesa_vendor_dlkm_dist
    pkg_install(
        name = "imx937_mesa_vendor_dlkm_dist",
        srcs = [
            ":imx937_kernel_files",
            ":imx937_mesa_vendor_dlkm_files",
        ],
...
IMX937_VENDOR_RAMDISK_MODULES = _IMX937_VENDOR_RAMDISK_MODULES
IMX937_VENDOR_DLKM_MODULES = _IMX937_VENDOR_DLKM_MODULES
IMX937_FIRST_STAGE_MODULES = _IMX937_FIRST_STAGE_MODULES
IMX937_RECOVERY_ADDITION_MODULES = _IMX937_RECOVERY_ADDITION_MODULES
IMX937_MALI_GPU_MODULES = _IMX937_MALI_GPU_MODULES
IMX937_IN_TREE_MODULES = _IMX937_IN_TREE_MODULES
IMX937_EXT_MODULES = _IMX937_EXT_MODULES
IMX937_MODULES_LIST = _IMX937_MODULES_LIST
IMX937_MODULES_RECOVERY_LIST = _IMX937_MODULES_RECOVERY_LIST
IMX937_MODULES_CHARGER_LIST = _IMX937_MODULES_CHARGER_LIST
IMX937_DTB_OUTS = _IMX937_DTB_OUTS
