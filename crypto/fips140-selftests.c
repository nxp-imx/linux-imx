 ...
 * is somewhat helpful.  Basically, all reachable implementations of all FIPS
 * approved algorithms (including modes of operation) must be tested.  However:
 *
 ...
 *
 * In the case of the traditional approach where multiple implementations of a
 * single algorithm can be registered with the generic crypto infrastructure,
 * all such implementations are considered reachable regardless of their
 * cra_priority value.  The FIPS testing requirement applies to each.
 *
 * In the case of the modern approach where a (final) selection between multiple
 * code paths in an algorithm's implementation is made at module load time based
 * on CPU features, this is considered a single implementation for FIPS
 * purposes.  Only the single API for the algorithm needs to be tested.
 *
 * Some algorithms (e.g. SHA-1, SHA-256, SHA-512) have FIPS self-tests upstream
 * in lib/crypto/ already.  These tests are sufficient for fips140.ko, which
 * executes them when it calls the initcalls.
 *
 * This file implements the tests for the remaining algorithms.  See
 * fips140_selftests[] for the list of tests we've selected.  Most of the test
 * vectors were generated by the script tools/crypto/gen_fips140_testvecs.py,
 * using the known-good implementations in the Python packages hashlib,
 * pycryptodome, and cryptography.
 *
 * There are several reasons why we can't reuse the upstream crypto/testmgr.c:
 *
...
static const struct fips_test fips140_selftests[] __initconst = {
	...
	/* HMAC-SHA256 test is in sha256_mod_init() in lib/crypto/sha256.c */
	/* HMAC-SHA512 test is in sha512_mod_init() in lib/crypto/sha512.c */
