...
static int imx_pcm512x_probe(struct platform_device *pdev)
{
	...
	data->dai_link[0].codecs->dai_name = "rpmsg-pcm512x-hifi";
	data->dai_link[0].dai_fmt = data->daifmt;


	if (of_property_read_bool(pdev->dev.of_node, "rpmsg-out"))
		data->dai_link[0].capture_only = false;

	if (of_property_read_bool(pdev->dev.of_node, "rpmsg-in"))
		data->dai_link[0].playback_only = false;

	if (data->dai_link[0].playback_only && data->dai_link[0].capture_only) {
